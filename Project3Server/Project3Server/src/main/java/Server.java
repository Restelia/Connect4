				...
				try {
					...
					switch (message.getType()) {
						...
						case MOVE:
							...
							Message timerMessage = new Message(MessageType.TURN_TIMER, Integer.toString(game.getTurnDurationSeconds()), "client" + nextPlayer);
							if (nextOut != null) nextOut.writeObject(turnMessage);
							if (nextOut != null) nextOut.writeObject(timerMessage);
						...
						case REMATCH:
							...
							if (rematchRequests.containsKey(opponentT) && rematchRequests.get(opponentT) == lastGameId) {
								...
								// Create a new game but keep the same settings (e.g., turn duration, initial starting player)
								Connect4Game newGame = new Connect4Game();
								newGame.setTurnDuration(lastGame.getTurnDurationSeconds());  // Retain the same turn duration
								newGame.setPlayers(count, opponentT);  // Set the same players
								newGame.resetTimeouts();  // Reset timeout counters
								newGame.setGameFinished(false);  // Make sure the game is not marked as finished
